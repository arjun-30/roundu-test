...
const Jobs = () => {
  ...
  const filteredJobs = useMemo(() => {
    ...
    return {
      upcoming: providerRequests.filter(
        r =>
          [
            "pending",
            "accepted",
      ...
      active: providerRequests.filter(
        r =>
          [
            "on_the_way",
            "arrived",
    ...
    <div className="min-h-full flex flex-col bg-background pb-24 relative">
      ...
      <div className="px-5 mt-6 flex gap-2">
        <button
          ...
          className={`flex-1 py-2.5 text-[11px] font-bold rounded-xl transition-all ${tab === "upcoming" ? "bg-primary text-primary-foreground shadow-sm" : "bg-input text-muted-foreground"
            }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2.5 text-[11px] font-bold rounded-xl transition-all ${tab === "active" ? "bg-primary text-primary-foreground shadow-sm" : "bg-input text-muted-foreground"
            }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2.5 text-[11px] font-bold rounded-xl transition-all ${tab === "completed" ? "bg-primary text-primary-foreground shadow-sm" : "bg-input text-muted-foreground"
            }`}
        >
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className={`text-[10px] font-bold px-2 py-1 rounded-md ${job.status === 'in_progress' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'}`}>
                        ...
                        {job.status === "in_progress" && "Working"}
                        {job.status === "completed" && "Completed"}
                        {job.status === "payment_pending" && "Payment Pending"}
                      </span>
                    </div>
                    <p className="text-xs text-muted-foreground mb-3">{job.customerName}</p>
