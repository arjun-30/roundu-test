...
import {
  ...
  FileX, ShieldOff, HelpCircle, ArrowRight, Phone,
} from "lucide-react";
import { useApp } from "@/context/AppContext";
import { useProviderApprovalStatus } from "@/hooks/useProviderApprovalStatus";

// Use real admin email — change before production
const SUPPORT_EMAIL = "admin@roundu.com";
const SUPPORT_PHONE = "+91 99999 00000"; // dummy for testing

export default function ApplicationRejected() {
  const navigate = useNavigate();
  const { user, dispatch } = useApp();
  const { status, rejectionReason, loading } = useProviderApprovalStatus(user?.id ?? null);

  // Auto-redirect if admin re-approves
  useEffect(() => {
  ...
  const handleContactAdmin = () => {
    const subject = encodeURIComponent(`Provider Account Review — ${user?.name ?? ""}`);
    const body = encodeURIComponent(
      `Hello RoundU Admin Team,\n\nMy name is ${user?.name ?? ""}. I would like to request a review of my provider application.\n\nPhone: ${user?.phone ?? ""}\nUser ID: ${user?.id ?? ""}\n\nPlease assist me with the next steps.\n\nThank you.`
    );
      ...
      <div className="relative z-10 w-full max-w-[420px]">
        ...
        >
          ...
          <div className="px-6 py-6 space-y-4">
            ...
            {/* Rejection reason — prominently shown when available */}
            {rejectionReason ? (
              <motion.div
              ...
              <div className="border-l-4 border-slate-300 bg-slate-50 rounded-r-2xl px-4 py-3">
                <div className="flex items-center gap-2 mb-1">
                  <FileX size={14} className="text-slate-400" />
                  <span className="text-xs font-bold text-slate-500 uppercase tracking-wide">Rejection Reason</span>
                </div>
                <p className="text-sm text-slate-500">No reason was provided. Please contact the admin team.</p>
              </div>
            )}

            {/* Possible reasons */}
            <div className="space-y-1.5">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide">This may be due to:</p>
              {[
            ...
            <div className="bg-slate-50 rounded-2xl p-4 space-y-3">
              ...
              {/* Phone (dummy for testing) */}
              <div className="flex items-center gap-2 justify-center text-xs text-slate-400">
                <Phone size={12} />
                <span>{SUPPORT_PHONE} &nbsp;·&nbsp; <span className="italic">test number</span></span>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-2xl p-4">
              ...
              <div>
                ...
                <p className="text-xs text-blue-600 leading-relaxed">
                  If all required information is provided correctly, the verification team will review your case within <strong>2 working days</strong>.
                </p>
              </div>
            </div>
