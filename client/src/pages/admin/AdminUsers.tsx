...
function exportCSV(data: User[]) {
    const headers = ["ID", "Name", "Phone", "Email", "Address", "Role", "Wallet", "Joined"];
    const rows = data.map(u => [
        u.id, u.full_name, u.phone, u.email, u.address, u.role,
        u.wallet_balance, new Date(u.created_at).toLocaleDateString("en-IN"),
    ]);
    ...
    return (
        <>
            ...
            >
                ...
                <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="font-semibold text-slate-700 text-sm">{user.full_name || "—"}</span>
                    </div>
                </td>
                ...
                <td className="px-4 py-3 text-sm font-semibold text-slate-700">₹{(user.wallet_balance ?? 0).toLocaleString()}</td>
            ...
            <AnimatePresence>
                {open && (
                    <tr>
                        <td colSpan={8} className="px-4 pb-3 bg-slate-50/60">
                            <motion.div
                ...
                <table className="w-full text-sm min-w-[750px]">
                    <thead>
                        <tr className="border-b border-slate-100 bg-slate-50">
                            {["User", "Phone", "Email", "Address", "Role", "Wallet", "Joined", ""].map(h => (
                    ...
                    <tbody>
                        {loading
                            ...
                            : paginated.length === 0
                                ? (
                                    <tr><td colSpan={8} className="px-4 py-12 text-center text-slate-400">No users found.</td></tr>
                                )
