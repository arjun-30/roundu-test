...
const ActingDriverBooking = () => {
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col bg-[#EEF2F7] font-sans text-[#030916]">
      ...
      <div className="px-5 pt-6 pb-4 flex items-center gap-3 bg-[#EEF2F7] sticky top-0 z-20">
        ...
        <div>
          <h1 className="text-[20px] font-extrabold text-[#030916] leading-tight">Acting Drivers</h1>
          <p className="text-[13px] text-gray-500 font-medium">Book your chauffeur</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 pt-2 pb-36 space-y-5">
        ...
        <div className="relative overflow-hidden rounded-[22px] bg-gradient-to-br from-[#152E4B] to-[#1C3D63] p-5 shadow-lg">
          ...
          <div className="absolute bottom-0 left-1/2 w-20 h-20 rounded-full bg-[#F59E0B]/10 -translate-x-1/2 translate-y-1/2" />
        ...
        {/* Pickup Location */}
        <div>
          <label className="flex items-center gap-2 text-[13px] font-extrabold text-[#030916] mb-2 px-1">
            <MapPin size={14} className="text-[#152E4B]" />
          ...
          <input
            ...
            className={`w-full bg-white rounded-[16px] px-4 py-3.5 text-[14px] font-medium text-[#030916] border-[1.5px] placeholder:text-gray-400 focus:outline-none transition-all shadow-sm ${
        ...
        {pickup.trim() && drop.trim() && (
          <div className="bg-white rounded-[16px] p-4 border-[1.5px] border-[#152E4B]/10 shadow-sm flex items-center gap-3 animate-fade-in">
            <div className="flex flex-col items-center gap-1 flex-shrink-0">
              <div className="w-2.5 h-2.5 rounded-full bg-[#152E4B]" />
              <div className="w-0.5 h-6 bg-dashed border-l border-dashed border-[#152E4B]/30" />
              <div className="w-2.5 h-2.5 rounded-full bg-[#F59E0B]" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[12px] font-bold text-[#030916] truncate">{pickup.trim()}</p>
              <p className="text-[11px] text-gray-400 my-1">to</p>
              <p className="text-[12px] font-bold text-[#030916] truncate">{drop.trim()}</p>
            </div>
        ...
        <div>
          <label className="flex items-center gap-2 text-[13px] font-extrabold text-[#030916] mb-2 px-1">
            <Calendar size={14} className="text-[#152E4B]" />
          ...
          <div className="flex items-center gap-4 bg-white rounded-[16px] px-4 py-3 shadow-sm border-[1.5px] border-transparent">
            <button
              ...
              className="w-9 h-9 rounded-full bg-[#EEF2F7] flex items-center justify-center text-[#152E4B] font-extrabold text-xl active:scale-90 transition-transform select-none"
            >
              −
            </button>
            <div className="flex-1 text-center">
              <span className="text-[26px] font-extrabold text-[#030916]">{days}</span>
              <span className="text-[13px] text-gray-400 font-medium ml-2">{days === 1 ? "day" : "days"}</span>
            </div>
            <button
              onClick={() => setDays(d => d + 1)}
              className="w-9 h-9 rounded-full bg-[#152E4B] flex items-center justify-center text-white font-extrabold text-xl active:scale-90 transition-transform select-none"
            >
              +
            </button>
          </div>
          ...
          {days >= 7 && (
            <p className="text-[11px] text-[#A95D06] font-semibold mt-1.5 px-1 flex items-center gap-1">
        ...
        <div>
          <label className="flex items-center gap-2 text-[13px] font-extrabold text-[#030916] mb-2 px-1">
            <FileText size={14} className="text-[#152E4B]" />
          ...
          <textarea
            ...
            className="w-full bg-white rounded-[16px] px-4 py-3.5 text-[14px] font-medium text-[#030916] border-[1.5px] border-transparent placeholder:text-gray-400 focus:outline-none focus:border-[#152E4B]/30 transition-all shadow-sm resize-none"
          />
        </div>

        {/* Trust Badges */}
        <div className="flex gap-4 items-end justify-between opacity-80 px-2 pt-2">
          <span className="flex flex-col items-center gap-1 text-[10.5px] font-bold text-gray-500 text-center uppercase tracking-wide">
            <ShieldCheck size={26} className="text-[#A95D06] mb-1" strokeWidth={1.5} />
            Verified Drivers
          </span>
          <span className="flex flex-col items-center gap-1 text-[10.5px] font-bold text-gray-500 text-center uppercase tracking-wide">
            <Clock size={26} className="text-[#152E4B] mb-1" strokeWidth={1.5} />
            On-time Arrival
          </span>
          <span className="flex flex-col items-center gap-1 text-[10.5px] font-bold text-gray-500 text-center uppercase tracking-wide">
            <ThumbsUp size={26} className="text-[#F59E0B] mb-1" strokeWidth={1.5} />
            100% Satisfaction
          </span>
        </div>
      </div>
      ...
      {/* ── Fixed CTA ── */}
      <div className="fixed bottom-0 left-0 right-0 max-w-[430px] mx-auto px-5 py-4 bg-[#EEF2F7] border-t border-[#E0E6EE]">
        <button
          onClick={handleFindDrivers}
          className="w-full py-4 rounded-[18px] bg-[#152E4B] text-white font-extrabold text-[15px] tracking-tight shadow-[0_4px_20px_rgba(21,46,75,0.35)] hover:bg-[#1C3D63] active:scale-[0.98] transition-all flex items-center justify-center gap-2"
        >
