...
export async function checkScheduleConflict(
  ...
  const activeBookings = await getProviderActiveBookings(providerId);
  const proposedEnd = new Date(proposedStart.getTime() + proposedDurationHours * 60 * 60 * 1000);

  for (const booking of activeBookings) {
    if (!booking.scheduled_at) continue;
    const bookingStart = new Date(booking.scheduled_at);
    if (isNaN(bookingStart.getTime())) continue;
    const bookingDurationHours = booking.duration || 2;
    const bookingEnd = new Date(bookingStart.getTime() + bookingDurationHours * 60 * 60 * 1000);

    // Check overlap: A < Y && X < B
    if (bookingStart < proposedEnd && proposedStart < bookingEnd) {
      return { 
        conflict: true, 
        message: `Schedule conflict: You have another job from ${bookingStart.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} to ${bookingEnd.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}.` 
      };
    }
  }
